...
class ReservationRepoDb extends ReservationRepo {
  ...
  Future<ReservationModel> createReservation({
    ...
    final response = await apiService.post('$_baseUrl/', data: data);
  ...
  @override
  Future<ReservationModel> getReservation({required int reservationId}) async {
    final response = await apiService.get('$_baseUrl/$reservationId');
    if (response.statusCode == 200) {
      return ReservationModel.fromJson(response.data);
    } else {
  ...
  Future<List<ReservationModel>> getAllReservation({int page = 1}) async {
    try {
      final response =
          await apiService.get('$_baseUrl/', query: {'page': page});
      if (response.statusCode == 200) {
  ...
  Future<ReservationModel> updateReservation({
    ...
    final reservationId = await Token.getReservtaionId();
    final data = {
      'user_id': userId,
      'table_id': tableId,
      'duration_hours': durationHours,
    ...
    final response =
        await apiService.put('$_baseUrl/$reservationId', data: data);
    if (response.statusCode == 200) {
      return ReservationModel.fromJson(response.data);
    } else {
      throw Exception('Failed to get reservations');
  ...
  Future<String> deleteReservation({required int reservationId}) async {
    final response = await apiService.delete('$_baseUrl/$reservationId');
    if (response.statusCode == 200) {
